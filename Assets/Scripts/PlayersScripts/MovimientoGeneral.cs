...
public class MovimientoGeneral : MonoBehaviour, IMovimiento
{
    ...
    PenduloInteraccion penduloSpt;

    public void MovimientoAdelante()
    {
        if (!penduloSpt.jugadorCortado)
        {
    ...
    {
        bool teclaAdelante = Input.GetKey(teclaMovimientoAdelante);
        bool teclaAtras = Input.GetKey(teclaMovimientoAtras);


        if (!moverseAtras && !moverseAdelante && !sinMovimiento)
        {
            if (teclaAdelante && !caminoBloqueado)
            {
                animator.SetTrigger("moverse");
    ...
    void Start()
    {
        ...
        penduloSpt = GetComponent<PenduloInteraccion>();
